...
public class RecipeControllerTest {
	...
	/**
	 * This is a simple test to see if the getRecipes method works for an uninitialized database.
	 */
	@Test
	@Transactional
	 ...
	 * @throws Exception an exception that occurs when the api request is invalid
	 */
	@Test
	@Transactional
	public void testCreateRecipe() throws Exception {
		// We can add three recipes, then delete them one by one and see what is left
		RecipeDto recipe1 = new RecipeDto(1L, "Mocha", 300, 3, 3, 3, 3);
		RecipeDto recipe2 = new RecipeDto(2L, "Coffee", 100, 1, 1, 1, 1);
		RecipeDto recipe3 = new RecipeDto(3L, "Black", 200, 2, 2, 2, 2);
		
		// We are going to get a string representation of the database through this standard function
		String recipe = mvc.perform(get("/api/recipes"))
				.andExpect(status().isOk())
				.andReturn().getResponse().getContentAsString();
		...
		assertEquals(recipe, "[]");
		// It makes more sense to say we dont have the recipe we are about to add
		assertFalse(recipe.contains("Mocha"));
		...
		recipe = mvc.perform(get("/api/recipes"))
				.andExpect(status().isOk())
				.andReturn().getResponse().getContentAsString();
		...
		mvc.perform(post("/api/recipes")
				.contentType(MediaType.APPLICATION_JSON)
				.content(TestUtils.asJsonString(recipe2))
				.accept(MediaType.APPLICATION_JSON));
		
		// Now we should update the string to have the recipe, this will have Mocha and hot chocolate
		recipe = mvc.perform(get("/api/recipes"))
				.andExpect(status().isOk())
				.andReturn().getResponse().getContentAsString();
		...
		// Next we can try to add a duplicate, this is not allowed so we check an error
		// Lets add hot chocolate again and see what happens, we will change status().isOk to test this
		mvc.perform(post("/api/recipes")
				.contentType(MediaType.APPLICATION_JSON)
				.content(TestUtils.asJsonString(recipe2))
				.accept(MediaType.APPLICATION_JSON))
		...
		mvc.perform(post("/api/recipes")
				.contentType(MediaType.APPLICATION_JSON)
				.content(TestUtils.asJsonString(recipe3))
				.accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
		...
		// Finally we will create a drink that overloads the system since 3 is max
		RecipeDto futureDrink = new RecipeDto(2L, "nnnnnn", 250, 5, 1, 0, 0);
		
		// The standard post function
		mvc.perform(post("/api/recipes")
				.contentType(MediaType.APPLICATION_JSON)
				.content(TestUtils.asJsonString(futureDrink))
				.accept(MediaType.APPLICATION_JSON))
	 ...
	 *
	 * @throws Exception an exception that happens if the API request is invalid
	 */
	@Test
	@Transactional
	public void deleteRecipes() throws Exception {
		...
		mvc.perform(delete("/api/recipes/{id}", ids[0])
				.contentType(MediaType.APPLICATION_JSON)
				.content(TestUtils.asJsonString(recipe1))
				.accept(MediaType.APPLICATION_JSON));
		...
		// Since we deleted the first one it should be gone and the others should still be there
		...
		mvc.perform(delete("/api/recipes/{id}", ids[2])
				.contentType(MediaType.APPLICATION_JSON)
				.content(TestUtils.asJsonString(recipe3))
				.accept(MediaType.APPLICATION_JSON));
		...
		// And the last one
		mvc.perform(delete("/api/recipes/{id}", ids[1])
				.contentType(MediaType.APPLICATION_JSON)
				.content(TestUtils.asJsonString(recipe2))
				.accept(MediaType.APPLICATION_JSON));
	...
	public void getRecipe() throws Exception {
		...
		String recipe1String = mvc.perform(get("/api/recipes/" + recipe1.getName()))
				.andExpect(status().isOk())
				.andReturn().getResponse().getContentAsString();
		...
		String recipe2String = mvc.perform(get("/api/recipes/" + recipe2.getName()))
				.andExpect(status().isOk())
				.andReturn().getResponse().getContentAsString();
		...
		String recipe3String = mvc.perform(get("/api/recipes/" + recipe3.getName()))
				.andExpect(status().isOk())
				.andReturn().getResponse().getContentAsString();
